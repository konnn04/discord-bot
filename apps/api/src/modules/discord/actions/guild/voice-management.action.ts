  ...
  parameters: {
    ...
    properties: {
      channel_id: {
        type: 'string',
        description:
          'ID kênh thoại muốn xem (bỏ trống để lấy kênh thoại của người ra lệnh)',
      },
    },
  },
};
...
export async function getVoiceMembersAction(
  ...
  if (args.channel_id) {
    const ch = await ctx.guild.channels
      .fetch(args.channel_id)
      .catch(() => null);
  ...
  if (!voiceChannel) {
    return fail(
      'Bạn chưa ở trong kênh thoại nào và không chỉ định kênh thoại hợp lệ.',
    );
  }
...
export async function moveVoiceMembersAction(
  ...
  if (!targetChannel) {
    targetChannel = (await ctx.guild.channels
      .fetch(query)
      .catch(() => null)) as VoiceBasedChannel | undefined;
  }
  ...
  if (!rawIds || rawIds.toLowerCase() === 'all') {
    if (!currentChannel) {
      return fail(
        'Bạn không ở trong phòng thoại nào để chuyển tất cả mọi người.',
      );
    }
  ...
  if (membersToMove.length === 0) {
    return fail(
      'Không tìm thấy thành viên nào đang ở trong voice để di chuyển.',
    );
  }
