...
export class MeetingTracker {
  ...
  createSession(
    ...
    const existingSession = this.getSessionByVoiceChannel(
      guildId,
      voiceChannelId,
    );
    if (existingSession) {
      throw new Error(
        'This voice channel already has an active tracking session',
      );
    }
  ...
  addParticipant(voiceChannelId: string, member: GuildMember): void {
    ...
    if (existing) {
      existing.sessions.push({ joinedAt: now, leftAt: null });
    } else {
  ...
  async endSession(voiceChannelId: string): Promise<MeetingSession | null> {
    await Promise.resolve(); // Future DB integration
    const session = this.sessions.get(voiceChannelId);
